...
export class Contract  {
  ...
  async getStakersBatch(_start: ethers.BigNumber, _offset: ethers.BigNumber): Promise<{_amount: Array<ethers.BigNumber>,_tiers: Array<ethers.BigNumber>,_addr: Array<string>}> {
    const result = await this.call("getStakersBatch", [_start, _offset])
    return  {
      _amount: result[0],
      _tiers: result[1],
      _addr: result[2],
  ...
  async getStakersLength(): Promise<ethers.BigNumber> {
    const result = await this.call("getStakersLength", [])
    return result[0]
  }

  async getTier(_addr: string): Promise<ethers.BigNumber> {
    const result = await this.call("getTier", [_addr])
    return result[0]
  }

  async getTierCfg(): Promise<{tempTier: Array<ethers.BigNumber>,tempWeight: Array<ethers.BigNumber>}> {
    const result = await this.call("getTierCfg", [])
    return  {
      tempTier: result[0],
      tempWeight: result[1],
    }
  ...
  async myStakeInfo(): Promise<{_staked: ethers.BigNumber,_earned: ethers.BigNumber,_stakingTime: ethers.BigNumber,_unlockTime: ethers.BigNumber,_tier: ethers.BigNumber}> {
    const result = await this.call("myStakeInfo", [])
    return  {
      _staked: result[0],
      _earned: result[1],
      _stakingTime: result[2],
      _unlockTime: result[3],
      _tier: result[4],
    }
  }
  ...
  async stakers(null: ethers.BigNumber): Promise<string> {
    const result = await this.call("stakers", [null])
    return result[0]
  }

  async stakingLockPeriod(): Promise<ethers.BigNumber> {
    const result = await this.call("stakingLockPeriod", [])
    return result[0]
  }
  ...
  async tiersCfg(null: ethers.BigNumber): Promise<ethers.BigNumber> {
    const result = await this.call("tiersCfg", [null])
    return result[0]
  }

  async tiersWeightCfg(null: ethers.BigNumber): Promise<ethers.BigNumber> {
    const result = await this.call("tiersWeightCfg", [null])
    return result[0]
  }
  ...
  async unlockTime(null: string): Promise<ethers.BigNumber> {
    const result = await this.call("unlockTime", [null])
    return result[0]
  }

  async userStakeInfo(_user: string): Promise<{_staked: ethers.BigNumber,_earned: ethers.BigNumber,_stakingTime: ethers.BigNumber,_unlockTime: ethers.BigNumber,_tier: ethers.BigNumber}> {
