        ...
        for (const item of block.items as unknown as Array<BatchBlock<Item> & { name: string; event: any }>) {
            if (item.name === 'EVM.Log') {
                ...
                switch (evtAddr) {
                    ...
                    case AVAULT_SWAP:
                        const opts = {
                            pairAddress: evtAddr === NASTR_SWAP ? ASTR_USDC_PAIR : '',
                        }
                        switch (topic) {
